...
func checkCmd(m []byte) {
	...
	if strings.HasPrefix(sl, "open") {
		...
		go func() {
			...
			// --- TFG
			if tool == "platformio" && len(args) == 2 {
				cmd := exec.Command("pip", "install", "platformio")
				...
				go func() {
					for scanner.Scan() {
						h.sendToRegisteredConnections([]byte(scanner.Text()))
					}
				}()
				err = cmd.Run()
				if err != nil {
...
func getUpload(serial_port string, fqdn string, data string) {
	rawDecodedText, err := base64.StdEncoding.DecodeString(data)
	go spClose2(serial_port)
	...
	usr, _ := user.Current()
	path := usr.HomeDir + "/.arduino-create"
	...
	cmd := exec.Command(tool, command1, command2, "-d", dir, "--board", fqdn)
	...
	command1 = "run"
	command2 = "upload"
	cmd = exec.Command(tool, command1, "--upload-port", serial_port, "-d", dir, "-t", command2)
	//cmd.Stdout = os.Stdout
	//cmd.Stderr = os.Stderr
	cmdReader, err = cmd.StdoutPipe()
	if err != nil {
	...
	if err != nil {
		log.Println(err)
		h.sendToRegisteredConnections([]byte("Error compiling and loading image"))
	}
...
func getLib(s1 string, s2 string) {
	...
	cmd := exec.Command(tool, "pkg", s1, "-g", "-l", s2)
	cmdReader, err := cmd.StdoutPipe()
	if err != nil {
		fmt.Fprintln(os.Stderr, "Error creating StdoutPipe for Cmd", err)
		return
