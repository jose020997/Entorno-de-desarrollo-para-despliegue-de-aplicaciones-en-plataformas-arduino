...
*/
function saveCode() {
  var fileName = window.prompt(lang_obj.savecode_text, 'BlocklyDuino')
...
function save() {
  ...
  var fileName = window.prompt(lang_obj.save_text, 'BlocklyDuino');
  // Store data in blob.
...
function discard() {
  var count = Blockly.mainWorkspace.getAllBlocks().length;
  if (count < 1 || window.confirm(lang_obj.discard_text + count + ' total)')) {
    Blockly.mainWorkspace.clear();
    renderContent();
  }
}
...
function uploadClick(type) {
    ...
    if (window.confirm("¿Preparado para cargar en Arduino?")){
        uploadCode(code, function(status, errorInfo) {
            if (status == 200) {
                alert("Programa cargado OK");
            } else {
